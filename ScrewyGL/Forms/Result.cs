    ...
    public partial class Result : Form
    {
        ...
        private void BTN_Download_Click(object sender, EventArgs e)
        {
            using (FolderBrowserDialog browserDG = new FolderBrowserDialog())
            {
                ...
                {
                    using (StreamWriter data = new StreamWriter(Path.Combine(browserDG.SelectedPath, "ScrewyGL_Result.txt")))
                    {
                        data.Write(RTB_Content.Text);
                        MessageBox.Show("Saved in: " + browserDG.SelectedPath, "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
        }

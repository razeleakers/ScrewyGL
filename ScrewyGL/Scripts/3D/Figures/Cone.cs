...
{
    public class SW_Cone3D : ScrewyFigure3D
    {
        private IntPtr _Quadric;

        public SW_Cone3D(string figureName, Color figureColor, IntPtr quadric) : base(figureName, figureColor)
        {
            this._PosY -= 1.0f;
            this._AngleX -= 90.0f;
        ...
        public override void RenderFigure(OpenGL sharpGL)
        {
            ...
            sharpGL.Cylinder(_Quadric, 1.0d, 0.0d, 2.0d, 32,32);
        ...
        {
            return $"{GetStartConfigData()} gluCylinder(quadric,1.0, 0.0, 2.0, 32,32);\r\n    gluDisk(quadric, 0.0, 1.0, 32, 1); {GetFinalConfigData()}";
        }
